...
{
    private const string HelperName = "MateMCP.WindowsCaptureHelper.exe";
    ...
    public bool Available
        => OperatingSystem.IsWindows() && File.Exists(ResolveHelperPath());

    public string BackendName
        => Available ? "windows-graphics-capture-stream" : "os-screenshot-fallback";
        ...
        lock (_sync)
        {
            ...
            try
            {
                ...
                var process = Process.Start(start)
                    ?? throw new InvalidOperationException("Could not start the Windows Graphics Capture preview helper.");
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (length <= 0 || length > DesktopVisionService.MaxCaptureBytes || width <= 0 || height <= 0)
                    throw new InvalidDataException("Windows Graphics Capture helper emitted an invalid frame header.");
    ...
    {
        var configured = Environment.GetEnvironmentVariable("MATEMCP_WINDOWS_CAPTURE_HELPER");
