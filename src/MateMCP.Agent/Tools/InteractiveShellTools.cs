    ...
    UserSecretStore secrets)
{
    ...
    [McpServerTool(Name = "shell_session_read"), Description("Reads terminal output produced by an interactive shell session since the supplied offset. Pass nextOffset from the previous response to receive only new output.")]
    ...
    public async Task<object> SendSecret(string sessionId, string credential, bool submit = true, CancellationToken cancellationToken = default)
    {
        ...
        if (!available.Any(x => string.Equals(x.Name, credential, StringComparison.OrdinalIgnoreCase)))
            throw new McpException($"Named credential '{credential}' does not exist.");

        var decision = await approvals.RequestAsync("secret.use", credential, $"Inject into shell session {sessionId[..Math.Min(8, sessionId.Length)]}: {Trim(command)}", cancellationToken);
        if (decision == ApprovalDecision.Deny)
        {
            await audit.WriteAsync("secret.use", $"{credential}:{sessionId}", "denied:approval", cancellationToken);
            throw new McpException("Credential use denied by local user.");
        }
        if (decision == ApprovalDecision.Timeout)
        {
            await audit.WriteAsync("secret.use", $"{credential}:{sessionId}", "denied:approval-timeout", cancellationToken);
            throw new McpException("Credential use approval timed out.");
        }

        var value = await secrets.ResolveAsync(credential, cancellationToken);
        if (value is null) throw new McpException($"Named credential '{credential}' could not be resolved from the local secure store.");
        try
        {
            await sessions.WriteSecretAsync(sessionId, value, submit, cancellationToken);
            await audit.WriteAsync("secret.use", $"{credential}:{sessionId}", "injected", cancellationToken);
            return new { sessionId, credential, injected = true, submit };
        }
        finally
        {
            // Strings cannot be reliably zeroed in managed memory. Never log or return value.
            value = null;
        }
    }
    ...
    [McpServerTool(Name = "secret_list"), Description("Lists locally configured credential names and descriptions. Secret values are never returned.")]
    public async Task<object> ListSecrets(CancellationToken cancellationToken = default)
    {
        ...
        return list.Select(x => new { x.Name, x.Description }).ToArray();
    }
