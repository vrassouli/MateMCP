...
## Generic shell architecture

- MateMCP exposes one generic interactive terminal abstraction for command-line programs: `shell_session_start`, `shell_session_read`, `shell_session_write`, `shell_session_send_secret`, and `shell_session_close`.
...
- For browser/native UI fields, use `ui_fill_secret` with a named credential and a uniquely matched semantic selector. Never read the secret into AI context or pass plaintext through `ui_type`/`keyboard_type`; the Agent resolves the value locally, binds the destination element before approval, and returns metadata only.
