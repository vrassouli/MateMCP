...
| --- | --- | --- | --- |
| Streamable HTTP MCP endpoint | Supported | Supported | Agent build and package smoke test |
| Loopback management UI and root redirect | Supported | Supported | Package smoke test checks `/health`, `/`, and `/ui` |
...
6. Confirm both macOS architectures (`osx-arm64`, `osx-x64`) and both Windows architectures (`win-x64`, `win-arm64`) produce installable artifacts.
